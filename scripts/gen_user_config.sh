...
#        sudo ./gen_user_config.sh bharat vmlinuz-6.6.0 initrd.img-6.6.0
#
#   3. With kernel .deb file:
#        sudo ./gen_user_config.sh chandan "" "" ./linux-image-6.6.0-custom.deb
#
# OUTPUT:
...
# sudo grub-reboot "User Kernels>${USER}: kernel-1"
# sudo reboot

menuentry "${USER}: kernel-1" {
	echo "Loading ${USER} ${KERNEL}"
	linux ${KERNEL} root=UUID=${UUID} ro quiet splash
	initrd ${INITRD}
}

menuentry "${USER}: kernel-2" {
	echo "Loading ${USER} ${KERNEL}"
	linux ${KERNEL} root=UUID=${UUID} ro drm.debug=0xe quiet splash
	initrd ${INITRD}
}
EOF
